...
values7{1} = A1(:, 6*mm+1:7*mm); values7{2} = A2(:, 6*mm+1:7*mm); values7{3} = A3(:, 6*mm+1:7*mm); %values1{4} = A4(:, 1:mm);
values8{1} = A1(:, 7*mm+1:8*mm); values8{2} = A2(:, 7*mm+1:8*mm); values8{3} = A3(:, 7*mm+1:8*mm);% values2{4} = A4(:, mm+1:2*mm);
values9{1} = A1(:, 8*mm+1:9*mm); values9{2} = A2(:, 8*mm+1:9*mm); values9{3} = A3(:, 8*mm+1:9*mm);% values3{4} = A4(:, 2*mm+1:3*mm);
values10{1} = A1(:, 9*mm+1:10*mm); values10{2} = A2(:, 9*mm+1:10*mm); values10{3} = A3(:, 9*mm+1:10*mm); %values4{4} = A4(:, 3*mm+1:end);
values11{1} = A1(:, 10*mm+1:11*mm); values11{2} = A2(:, 10*mm+1:11*mm); values11{3} = A3(:, 10*mm+1:11*mm); %values4{4} = A4(:, 3*mm+1:end);
values12{1} = A1(:, 11*mm+1:12*mm); values12{2} = A2(:, 11*mm+1:12*mm); values12{3} = A3(:, 11*mm+1:12*mm); %values4{4} = A4(:, 3*mm+1:end);
...
for kk=1:10;  %:20
 ...
 otherwise
  ...
  %Criterion 1
  %([norm(ttm(y,1,f)), norm(ttm(y,2,f)), norm(ttm(y,3,f))])
...
% standard query matching
  AA=[A1 A2 A3 ];
 % spy(abs(corrcoef(zscore(AA)))>0.2)
  x = AA\f;
  xr=reshape(x,m,ng); 
