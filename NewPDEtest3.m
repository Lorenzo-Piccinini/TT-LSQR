...
values4{1} = Phi3; values4{2} = Ni3; values4{3} = Psi3 * A4;
m=n1;

coeff{1}{1} = kron(values1{2}, values1{1});
coeff{1}{2} = values1{3};
coeff{2}{1} = kron(values2{2}, values2{1});
coeff{2}{2} = values2{3};
coeff{3}{1} = kron(values3{2}, values3{1});
coeff{3}{2} = values3{3};
coeff{4}{1} = kron(values4{2}, values4{1});
coeff{4}{2} = values4{3};
C1 = kron(b,a); C2 = c;
...
r = 50;
tic;
[X_1,X_2,r_res,a_res,rks,DD]=lsqr_matrix_multi(coeff,C1,C2,tol,imax,tol_tr,r);
t_matrix = toc;


semilogy(Res.real_rel)
hold on
semilogy(r_res.rel_res)
legend('TT','MATRIX')
