...
n = size(A_1,1);
AA = [A_1, A_2, A_3];
%spy(abs(corrcoef(zscore(AA)))>0.7)
%pause

for k = 1:l
    terms{k}{1} = A_1(:, (k-1)*mm+1:k*mm);
    %terms{k}{1} = terms{k}{1}/norm(terms{k}{1});
    terms{k}{2} = A_2(:, (k-1)*mm+1:k*mm);
    %terms{k}{2} = terms{k}{2}/norm(terms{k}{2});
    terms{k}{3} = A_3(:, (k-1)*mm+1:k*mm);
    %terms{k}{3} = terms{k}{3}/norm(terms{k}{3});
end
% Norms = [0; 0; 0];
% for k = 1:l
%     Norms(1) = Norms(1)+norm(terms{k}{1});
%     Norms(2) = Norms(2)+norm(terms{k}{2});
%     Norms(3) = Norms(3)+norm(terms{k}{3});
% end
...
Y = OpL(terms, X);
%q1 = ttm(ttm(Y,2,ones(784,1)/Norms(2)), 3, ones(784,1)/Norms(3));
%q2 = ttm(ttm(Y,1,ones(784,1)/Norms(1)), 3, ones(784,1)/Norms(3));
%q3 = ttm(ttm(Y,1,ones(784,1)/Norms(1)), 2, ones(784,1)/Norms(2));
